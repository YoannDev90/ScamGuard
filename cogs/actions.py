...
def _build_alert_embed(message: discord.Message, result: dict, title: str = None) -> discord.Embed:
    """Standard alert embed builder."""
    score = result.get("score", 0)
    embed = discord.Embed(
        title=title or f"Scam alert (score: {score})",
        colour=discord.Colour.dark_red() if score >= 70 else discord.Colour.red(),
    ...
    for action in actions:
        ...
        try:
            if atype == "delete":
                ...
                msg = action.get("message", "Votre message a été signalé.")
                try:
                ...
                if target:
                    embed = _build_alert_embed(message, result, title=f"🔴 Scam alert ({trigger})")
                ...
                if target:
                    embed = _build_alert_embed(message, result, title=f"📋 Log: {trigger}")
                    await target.send(embed=embed)
